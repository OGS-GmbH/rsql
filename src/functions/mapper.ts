...
import { customOperatorApi } from "../custom-operators";
import { type CustomOperatorDefinition, type CustomOperatorNode } from "../types/custom-operators";
...
function mapNode (astLike: IdNodeBase<unknown>): string {
  ...
  switch (astLike.id) {
    ...
    default: {
      ...
      const operatorDefinition: CustomOperatorDefinition | undefined = customOperatorApi.getOperator(astLike.id as string);

      if (operatorDefinition)
        queryString += operatorDefinition.toString(astLike as unknown as CustomOperatorNode);
      else
        throw new Error(` ${ astLike.id as string } is not registered. Make sure to register it first`);
